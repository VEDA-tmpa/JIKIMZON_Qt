...
MainWindow::MainWindow(QWidget *parent)
    ...
    , player(new VideoStreamPlayer(nullptr, this))
{
    qDebug() << "MainWindow 생성 시작";

    ui->setupUi(this);  // Set up the UI

    // 객체 초기화 완료 후 connect 호출
    ...
    if (!filePath.isEmpty()) {
        qDebug() << "비디오 파일 경로: " << filePath;
        player->processFile(filePath);
    } else {
        qDebug() << "비디오 파일 경로를 선택하지 않았습니다.";
    }
    ...
    qDebug() << "VideoStreamPlayer 시그널 연결 완료";

    // MetaDataDisplay 생성
    MetaDataDisplay* metaData = new MetaDataDisplay(this);
    // player = new VideoStreamPlayer(metaData, this);  // MetaDataDisplay 인스턴스를 전달
    qDebug() << "VideoStreamPlayer 생성 완료";
    ...
    qDebug() << "모드 전환 버튼 연결 완료";

    // 서버에 연결
    networkManager->connectToVideoServer("video_server_host", 12345);
    networkManager->connectToJsonServer("json_server_host", 54321);
    qDebug() << "서버 연결 시도 중";

    // 시그널 연결
    connect(networkManager, &NetworkManager::videoDataReceived, player, &VideoStreamPlayer::handleVideoData);
    connect(networkManager, &NetworkManager::jsonDataReceived, player, &VideoStreamPlayer::handleJsonData);
    qDebug() << "NetworkManager 시그널 연결 완료";

    eventLogManager = new EventLogManager("event/event_log.db", this);
    ...
    ui->sharpnessSlider->setRange(-100, 100);

    // 슬롯 연결 (여전히 필요)
    connect(ui->brightnessSlider, &QSlider::valueChanged, this, &MainWindow::onBrightnessSliderChanged);
    connect(ui->saturationSlider, &QSlider::valueChanged, this, &MainWindow::onSaturationSliderChanged);
    connect(ui->sharpnessSlider, &QSlider::valueChanged, this, &MainWindow::onSharpnessSliderChanged);
}
...
void MainWindow::onFrameReady(const QImage &image) {
    if (!image.isNull()) {
        currentFrame = image.copy();  // 새 프레임 저장

        // 슬라이더 값으로 밝기, 채도, 선명도 조정
        int brightness = ui->brightnessSlider->value();  // 밝기 슬라이더 값
        int saturation = ui->saturationSlider->value();  // 채도 슬라이더 값
        int sharpness = ui->sharpnessSlider->value();    // 선명도 슬라이더 값

        // 밝기 조정
        QImage adjustedImage = currentFrame.copy();
        applyBrightnessEffect(adjustedImage, brightness); // 밝기 효과 적용

        // 채도 조정
        applySaturationEffect(adjustedImage, saturation); // 채도 효과 적용

        // 선명도 조정
        applySharpnessEffect(adjustedImage, sharpness);   // 선명도 효과 적용

        // QLabel에 pixmap 설정
        QPixmap pixmap = QPixmap::fromImage(adjustedImage);
        ui->videoLabel->setPixmap(pixmap.scaled(ui->videoLabel->size(), Qt::KeepAspectRatio, Qt::SmoothTransformation));
        ui->videoLabel->update(); // QLabel 업데이트
...
void MainWindow::toggleMode() {
    ...
    qDebug() << "모드 전환: " << (isNightMode ? "Night Mode" : "Light Mode");
