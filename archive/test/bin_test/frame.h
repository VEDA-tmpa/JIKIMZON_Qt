...
public:
    ...
    bool Deserialize(const std::vector<uint8_t>& inBuffer) {
        ...
        // timestamp 초기화
        std::memcpy(timestamp.data(), mHeaderStruct.timestamp, sizeof(mHeaderStruct.timestamp));
        return true;
    }
...
public:
    ...
    void Serialize(std::vector<uint8_t>& outBuffer) const {
        ...
        if (!data.empty()) {
            ...
            switch (mHeader.GetHeaderStruct().imageFormat) {
            case ImageFormat::RAW:
                // RAW 형식 처리
                imageData.assign(data.data, data.data + data.total() * data.elemSize());
                break;
            case ImageFormat::JPEG:
                cv::imencode(".jpg", data, imageData);  // JPEG 형식으로 인코딩
                break;
            case ImageFormat::PNG:
                cv::imencode(".png", data, imageData);  // PNG 형식으로 인코딩
                break;
            }
    ...
    void Deserialize(const std::vector<uint8_t>& inBuffer) {
        ...
        std::vector<uint8_t> bodyBuffer(inBuffer.begin() + headerSize, inBuffer.end() - mHeader.GetHeaderStruct().bodySize);
        mBody.Deserialize(bodyBuffer);

        // 이미지 데이터 추출 (형식에 따라 처리)
        std::vector<uint8_t> imageData(inBuffer.end() - mHeader.GetHeaderStruct().bodySize, inBuffer.end());
        switch (mHeader.GetHeaderStruct().imageFormat) {
        case ImageFormat::RAW:
            data = cv::Mat(mHeader.GetHeaderStruct().imageHeight, mHeader.GetHeaderStruct().imageWidth, CV_8UC3, imageData.data());
            break;
        case ImageFormat::JPEG:
            data = cv::imdecode(imageData, cv::IMREAD_COLOR);  // JPEG 디코딩
            break;
        case ImageFormat::PNG:
            data = cv::imdecode(imageData, cv::IMREAD_COLOR);  // PNG 디코딩
            break;
        }
    }
