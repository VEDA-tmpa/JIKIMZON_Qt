...
void DashboardWidget::setupLineChart()
{
    ...
    chart->setBackgroundBrush(QBrush(QColor(255, 255, 255, 190))); // 배경 투명도 설정
    ...
    for (const QString &type : objectTypes) {
        ...
        // 선의 굵기 설정
        QPen pen(getObjectLineColor(type)); // 사용자 정의 색상 사용
        pen.setWidth(3); // 선 굵기 설정 (1이 기본값)
        pen.setStyle(Qt::SolidLine); // SolidLine, DashLine, DotLine 등 선택 가능
        series->setPen(pen); // 펜 적용
...
{
    // anomalyListWidget = new QListWidget(this);

    // anomalyListWidget->setStyleSheet(
    //     "QListWidget {"
    //     "    background-color: rgba(255, 255, 255, 100);"  // 배경 색상과 투명도
    //     "    border-radius: 10px;"  // 모서리 둥글게
    //     "    padding: 10px;"  // 내부 여백
    //     "}"
    //     "QListWidget::item {"
    //     "    padding: 8px;"  // 리스트 아이템 간격
    //     "    border-radius: 8px;"  // 아이템 모서리 둥글게
    //     "    background-color: rgba(0, 0, 0, 200);"  // 각 아이템 배경 색상
    //     "    border-bottom: 1px solid #e0e0e0;"  // 아이템 간 경계선
    //     "}"
    //     "QListWidget::item:hover {"
    //     "    background-color: #d3d3d3;"  // 마우스 호버 시 배경색
    //     "}"
    //     "QListWidget::item:selected {"
    //     "    background-color: rgba(168, 197, 218, 150);"  // 아이템 선택 시 배경 색상
    //     "    color: white;"  // 선택된 아이템 텍스트 색상
    //     "}"
    //     "QListWidget::item:selected:!active {"
    //     "    background-color: #3e8e41;"  // 선택된 아이템이 비활성일 때 색상
    //     "}"
    //     /* 세로 스크롤바 스타일 */
    //     "QScrollBar:vertical {"
    //     "    border: none;"
    //     "    background: rgba(176, 227, 255, 100);"  // 스크롤바 배경
    //     "    width: 12px;"  // 스크롤바 너비
    //     "    margin: 0px 0px 0px 0px;"
    //     "}"
    //     "QScrollBar::handle:vertical {"
    //     "    background-color: rgba(176, 227, 255, 100);"  // 스크롤바 핸들 색상
    ...
    // ui->anomalyListLayout->addWidget(anomalyListWidget);
}
...
void DashboardWidget::updateAnomalyList(int frameId, int detectedObjectCount, const QString &timestamp)
{
    ...
    if ((detectedObjectCount % 10) == 0) {
        ...
        // 리스트 위젯에 항목 추가
        ui->loglistWidget->addItem(anomaly);
        ...
        // 리스트 뷰에 새로 추가된 항목으로 스크롤 이동
        ui->loglistWidget->scrollToItem(ui->loglistWidget->item(ui->loglistWidget->count() - 1));
    }
}
